...
- **Plan 2 — `standards update` + partial class:** add managed-region sentinels to `AGENTS.md`/`CLAUDE.md`/`.github/copilot-instructions.md`; reclassify them as partial in `manifest.py`; implement `update` (hash-guard kit-tracked overwrite, managed-block splice, non-destructive `<path>.kit-<version>` sidecars, CHANGELOG-on-breaking print, leap-safe content compare).
